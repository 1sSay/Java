...
class Scanner {
    ...
    private boolean checkEndOfLine() {
        if (System.lineSeparator().length() == 1) {
            return System.lineSeparator().charAt(0) == this.lastSymbol;
        } else if (System.lineSeparator().length() == 2) {
            return (!this.token.isEmpty() &&
                    this.token.charAt(this.token.length() - 1) == System.lineSeparator().charAt(0) &&
                    this.lastSymbol == System.lineSeparator().charAt(1));
        } else {
            return false;
        }
    }
